...
import { searchApi, businessApi } from '../services/api';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const searchResponse = await searchApi.getSearches();
        setSearches(searchResponse.data.data);
        
        // Carregar todas as empresas
        const businessResponse = await businessApi.getBusinesses();
  ...
  const handleSearchSelect = async (searchId) => {
    try {
      ...
      const response = await searchApi.getSearchById(searchId);
